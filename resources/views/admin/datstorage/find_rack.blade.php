...
@section('content')
    ...
    <div class="search_wrap">
        ...
        <form action="" method="post">
            ...
            <table class="search_tab">
                    ...
                    <td><input type="button" value="添加架位" onclick=" window.parent.add_rack();"></td>
                </tr>
            ...
            <div class="result_popcontent">
                ...
                <table class="list_tab">
                    ...
                    <tr>
                        ...
                        @foreach($data as $v)
                            <tr>
                                <td><a href="javascript:void(0)" onclick="select('{{$v->Stor_code}}','{{$v->Stor_name}}')">{{$v->Stor_code}}</a></td>
                                <td>{{$v->Stor_name}}</td>
                                <td>
                                    @if($v->Stor_usable ==1)
    ...
    <script>
        function  select(Stor_code,Stor_name) {
            var index = parent.layer.getFrameIndex(window.name);

            parent.$('#rack').val(Stor_code);

            parent.layer.close(index);

        }
        function del(code,r) {
            layer.confirm('您确定删除该架位吗？', {
                btn: ['确认', '取消'] //按钮
            }, function () {
