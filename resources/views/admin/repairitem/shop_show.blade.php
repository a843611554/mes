...
@section('content')
    ...
    <div class="crumb_warp">
        ...
        <i class="fa fa-home"></i> <a href="{{url('admin/info')}}">首页</a> &raquo; <a href="{{url('admin/repairitem')}}">维修报表</a> &raquo; <a href="{{url('admin/repairitem/shop')}}">车间报表</a>&raquo;{{$shop->Dpt_Name}}
    </div>
    <!--面包屑导航 结束-->
    <!--搜索结果页面 列表 开始-->
                ...
                <tbody>
                    ...
                    @foreach($data as $v)
                        <tr>
                            ...
                            <td>@if(isset($v->createdate)){{date('Y-m-d',$v->createdate)}}@endif</td>
    ...
    <script>
        ...
        function edit(repair_bill) {
            layer.open({
                ...
                shadeClose:true,
                maxmin: true,
                content: "{{url('admin/repairitem/shop_show_edit')}}"+'/'+repair_bill,

                btn: ['确认', '取消'],
                yes: function(index){
                    var body = layer.getChildFrame('body', index); //得到iframe页的body内容

                    $.post("{{url('admin/repairitem/shop_show_update')}}",
                        {
                            '_token':"{{csrf_token()}}",
